        ...
        function makeCollector(serial) {
            ...
            const onData = (data) => {
                ...
                for (const line of parts) {
                    if (!line) continue;
                    let logged = false;
                    if (_config.filterLogcatKeywords && Array.isArray(_config.filterLogcatKeywords) && _config.filterLogcatKeywords.length > 0) {
                        for (const kw of _config.filterLogcatKeywords) {
                            if (!logged && line.includes(kw)) {
                                out('[ADB-' + serial + '] ' + line);
                                logged = true;
                            }
                        }
                        if (!logged && line.includes(appPackage)) {
                            out('[ADB-' + serial + '] ' + line);
                            logged = true;
                        }
                    ...
                    // capture bugreport on crash keywords
                    try {
                        if (line.includes('FATAL EXCEPTION') || line.includes('SIGSEGV') || (line.includes('ANR') && line.includes(appPackage)) || line.includes('Fatal signal')) {
                            if (!logged) out('[ADB-' + serial + '] ' + line);
                            if (crashCheck) {
